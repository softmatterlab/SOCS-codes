...
% check for particles closer that 1.5 sigma (to avoid overlap)
% nearest neighbours list... useful for sparing steps
[ nn_list ] = build_nn_list( x0, y0, 2*sigma );
crosscheck = zeros(Npart,Npart);
[ x0, y0, num_superp ] = hard_sphere_correction( x0, y0, 1.5*sigma, 0.001*sigma, nn_list, crosscheck );

fprintf('BEGIN: %d superpositions found\n',num_superp);

counter=0;
while (num_superp>0) && (counter<MAX_LOOP)
    [ x0, y0, num_superp ] = hard_sphere_correction( x0, y0, 1.5*sigma,  0.001*sigma, nn_list, crosscheck );
    counter = counter+1;
end
fprintf('BEGIN: %d cycles to correct superpositions\n',counter);
...
for i=1:Niter
    ...
    v(i,:) = sqrt(v2);
    K(i) = 1/2*m*sum(v2);
    T(i) = 1/2*m*mean(v2)/(1/2*PhysConst.kB)/2; % the last two is the dimension of the system
    E(i) = U(i)+sum(K(i));
...
%% 
clear all;
load('simulation_big_for_figure_smaller.mat','dt','x','y','vx','vy','Lx','Ly','Bx','By',...
